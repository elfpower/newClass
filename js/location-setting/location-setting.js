...
( function ( $, win, doc ) {
  ...
  var oWrap = $( ".loaction-setting" ); // 样式命名空间
  ...
  p.createCityPickerBox = function () { // 列表外层
    var proBox = "<div class='picker-box'></div>";
    oWrap.append( proBox );
  };

  p.createProList = function () { // 省列表项
  ...
  p.createCityList = function ( letter, pro ) { // 城市列表
  ...
  p.cityClick = function () { // 城市点击
    ...
    $( ".city-picker" ).on( "click", function ( e ) {
      ...
      if ( $( target ).is( "li" ) ) {
        ...
        //
        console.log( "这里传值给后台，并且跳转页面" )
        console.log(that.city)

        myJS.fixText("已选择"+that.city+",即将跳转到"+that.city+"的课程页面",function () {
          // 模拟数据
          window.location.href = "../../pages/curriculum/curriculum.html?city="+ that.city +"";
        })
  ...
  p.createNavBar = function () { // 右边索引导航
  ...
  p.navEvent = function () { // 索引对应 与 样式变换
    var that = this;
    var navBar = $( ".navbar" );
  ...
  p.createLetterPrompt = function ( letter ) { // 居中字母提示
    var prompt = $( ".prompt" );
    if ( prompt[ 0 ] ) {
      prompt.show();
    } else {
